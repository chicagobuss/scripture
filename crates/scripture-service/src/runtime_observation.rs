...
use scripture::{DriverError, Receipt, ReceiptFuture};
...
impl RuntimeObservationSession {
    ...
    pub fn observe_receipt(
        ...
        tokio::spawn(async move {
            ...
            match receipt.await {
                Ok(receipt) => {
                    observer.committed_ack(&ctx, &receipt, sequence);
                    let _ = sender.send(Ok(receipt));
                }
                Err(error) => {
                    observer.receipt_denied(&ctx, &driver_error_label(&error), sequence);
