//! Legacy v0 journal actor over [`scripture::JournalWriter`].
//!
//! Deprecated: new work targets [`crate::ChunkJournalService`] and
//! [`scripture::ChunkDriverHandle`]. This module remains for lab adapters until
//! equivalence evidence allows removal.
...
pub enum ServiceError {
    ...
    InvalidRequest,
    /// The bounded submission queue is closed or the actor has terminally failed.
...
#[deprecated(
    since = "0.2.0",
    note = "use ChunkJournalService and ChunkDriverHandle instead"
)]
...
mod tests {
    ...
    fn record(number: i64) -> Record {
        Record::new(
            ...
            bytes::Bytes::from(format!("record-{number}")),
        )
    }
    ...
    #[tokio::test]
    #[allow(deprecated)]
    async fn sealed_failure_transitions_the_actor() {
        let drive = Arc::new(InMemoryLogDrive::new()) as Arc<dyn LogDrive>;
        let log = AtomicLog::builder(drive, 4).build().expect("build log");
