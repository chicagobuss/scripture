...
impl RunLifecycle {
    ...
    pub fn deploy_actor_a(
        ...
        self.deploy_actor_a_with_faults(profile, scenario, ActorFaultEnv::default())
    }

    /// Deploys actor A with optional campaign-fault env and Ready expectation.
    pub fn deploy_actor_a_with_faults(
        &self,
        profile: &RustFsHomeFleetProfile,
        scenario: &str,
        faults: ActorFaultEnv,
    ) -> Result<ActorPlacement, LifecycleError> {
        let require_ready = faults.require_ready;
        self.deploy_actor(
            profile,
            scenario,
            ActorDeploy {
    ...
    pub fn deploy_actor_b_promote(
        ...
        self.deploy_actor_b_promote_with_faults(
            profile,
            scenario,
            candidate_term,
    ...
    /// Deploys promote actor B with optional campaign-fault env (family 15).
    pub fn deploy_actor_b_promote_with_faults(
        &self,
        profile: &RustFsHomeFleetProfile,
        scenario: &str,
        candidate_term: u64,
    ...
    /// Copies campaign NDJSON trace from a live actor pod before termination.
    pub fn collect_actor_trace(
        &self,
        actor: &str,
        dest: &std::path::Path,
        ...
        let remote = format!(
            "{}/{}:/campaign-artifacts/trace.ndjson",
            self.namespace, actor
        );
        let result = std::process::Command::new("kubectl")
            .arg("--context")
            .arg(&self.kube_context)
            .args(["cp", &remote, &dest.display().to_string()])
            .output()
            .map_err(|error| LifecycleError::Kubectl(error.to_string()))?;
        if !result.status.success() {
            return Err(LifecycleError::Kubectl(format!(
                "kubectl cp trace failed: {}",
    ...
    /// Fetches recent actor logs (redacted by caller policy).
    pub fn actor_logs(&self, actor: &str) -> Result<String, LifecycleError> {
        refuse_outside_namespace_mutations(&self.namespace)?;
        kubectl(
            &self.kube_context,
            &["-n", &self.namespace, "logs", actor, "--tail=200"],
        )
    }
    ...
    fn deploy_actor(
        ...
        if spec.require_ready {
            ...
            // Give the process time to attempt activation / fail closed.
            std::thread::sleep(Duration::from_secs(8));
        }
        actor_placement(&self.kube_context, &self.namespace, spec.name, spec.role)
    }

    /// Public HA prefix for a scenario (oracle + artifacts).
    #[must_use]
    pub fn scenario_ha_prefix(&self, scenario: &str) -> String {
        self.ha_prefix(scenario)
    }
...
pub(crate) fn prove_cleanup_independence(
    ...
    let proxy = child.deploy_toxiproxy().ok();
    ...
    if let Some(ref proxy) = proxy {
            ...
            .any(|name| name == "toxiproxy")
        {
            return Err(LifecycleError::Isolation(
                "toxiproxy service missing from inventory after deploy".into(),
            ));
        }
        let _ = proxy;
    }
    child.cleanup_and_wait(Duration::from_secs(180))?;
    let still_present = kubectl(
        &profile.kube_context,
        &["get", "namespace", &child_ns, "-o", "name"],
    )
    .is_ok();
    if still_present {
        return Err(LifecycleError::Timeout(format!(
            "cleanup independence: namespace {child_ns} still present"
        )));
    }
    Ok(serde_json::json!({
        ...
        "toxiproxy_deployed": proxy.is_some(),
...
pub struct ActorFaultEnv {
    /// `SCRIPTURE_FAULT_DIE_AFTER_PAYLOAD=1`
    pub die_after_payload: bool,
    /// Post-Serving LogDrive writes to skip before DieAfterPayload arms.
    pub die_after_payload_skip: u64,
    /// `SCRIPTURE_FAULT_ROOT_CAS_REPLY_LOSS=1`
    pub root_cas_reply_loss: bool,
    /// When false, do not wait for Ready (expected fail-closed activation).
    pub require_ready: bool,
}

impl ActorFaultEnv {
    /// DieAfterPayload after `skip` post-Serving durable writes (family 14).
    #[must_use]
    pub fn die_after_payload(skip: u64) -> Self {
        Self {
            die_after_payload: true,
            die_after_payload_skip: skip,
            root_cas_reply_loss: false,
            require_ready: true,
        }
    }

    /// Root-CAS reply loss on B promote (family 15); CLI retries once after Indeterminate.
    #[must_use]
    pub fn root_cas_reply_loss() -> Self {
        Self {
            die_after_payload: false,
            die_after_payload_skip: 0,
            root_cas_reply_loss: true,
            require_ready: true,
        }
    }

    /// Root-CAS reply loss expected to leave the actor not Ready (fail-closed probe).
    #[must_use]
    pub fn root_cas_reply_loss_fail_closed() -> Self {
        Self {
            die_after_payload: false,
            die_after_payload_skip: 0,
            root_cas_reply_loss: true,
  ...
  egress:
    ...
    - to:
      ...
      ports:
        - protocol: TCP
          port: 9000
    # Scenario-owned Toxiproxy listen port (family 16 directional fault)
    - to:
        - podSelector:
            matchLabels:
...
fn actor_pod_yaml(spec: &ActorPodSpec<'_>) -> String {
    let args_yaml = spec
        ...
        .collect::<Vec<_>>()
        .join("\n");
    let mut fault_env = String::new();
    fault_env.push_str(
        "        - name: SCRIPTURE_CAMPAIGN_TRACE_PATH\n          value: /campaign-artifacts/trace.ndjson\n",
    );
    fault_env.push_str(&format!(
        "        - name: SCRIPTURE_CAMPAIGN_RUN_ID\n          value: {run_id}\n",
        run_id = spec.run_id,
    ));
    fault_env.push_str(&format!(
        "        - name: SCRIPTURE_CAMPAIGN_ACTOR_ID\n          value: {name}\n",
        name = spec.name,
    ));
    if spec.faults.die_after_payload {
        fault_env.push_str(
            "        - name: SCRIPTURE_FAULT_DIE_AFTER_PAYLOAD\n          value: \"1\"\n",
        );
        fault_env.push_str(&format!(
            "        - name: SCRIPTURE_FAULT_DIE_AFTER_PAYLOAD_SKIP\n          value: \"{}\"\n",
            spec.faults.die_after_payload_skip
        ));
    }
    if spec.faults.root_cas_reply_loss {
        fault_env.push_str(
            "        - name: SCRIPTURE_FAULT_ROOT_CAS_REPLY_LOSS\n          value: \"1\"\n",
        );
    }
    format!(
        r#"apiVersion: v1
