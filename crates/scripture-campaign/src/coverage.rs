...
pub fn family_catalog() -> Vec<CoverageRow> {
    vec![
        ...
        // Families 12–17: process-separated producer→actor path (WP06).
        row(
            12,
            "process-separated-baseline",
            CoverageLayer::Resilience,
            Some(Scenario::RawLinesBaseline),
        ),
        row(
            13,
        ...
        row(
            ...
            CoverageLayer::Resilience,
            Some(Scenario::RawLinesDieAfterPayload),
        ),
        row(
            15,
            "root-cas-reply-loss-reread",
            CoverageLayer::Resilience,
            Some(Scenario::RawLinesRootCasReplyLoss),
        ),
        row(
            16,
            "directional-backend-loss-recovery",
            CoverageLayer::Resilience,
            Some(Scenario::RawLinesDirectionalLoss),
        ),
        row(
            17,
            "scoped-credential-invalidation",
            CoverageLayer::Resilience,
            Some(Scenario::RawLinesCredentialInvalidation),
        ),
        row(
            18,
...
fn row(
    ...
    CoverageRow {
        ...
        reason: match (family, scenario) {
            (13 | 14, _) => default_not_run_reason(family),
            (_, None) => default_not_run_reason(family),
            _ => None,
        },
        artifact_path: None,
