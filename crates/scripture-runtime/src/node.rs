...
impl VerseNodeSupervisor {
    ...
    /// Activates an empty open generation (greenfield post-bootstrap).
    ///
    /// The method enforces strict safety invariants:
    /// - It requires the current disposition of the local active generation to be
    ///   `RecoveryRequired` with `MustSealAndReplace`.
    /// - It requires the observed durable tail of the active generation to be exactly zero.
    /// - It requires the local node to be the named owner in the Canon register.
    /// - It fails closed without mutating if any invariant is violated.
    /// - The successor writable state is installed only after reconfiguration is applied.
        ...
        T: Timer + Clone + Send + 'static,
    {
        ...
        // Retrieve active and parts from store
        let (active, parts) = {
            let store = self.store.lock().await;
            let active = store
                .active
        ...
        let disposition = refuse_open_writable_reattach(active.clone(), parts.components(k)).await;
        let _observed_tail = match disposition {
            ...
            other => {
                return Err(SupervisorError::InvalidActivationDisposition { disposition: other });
            }
        };

        // Observe the current Canon and require the local node to be the named owner
        ...
        // Fail closed for stale local active state
        let observed_active = observed
            .observed()
            .state
            .active()
        ...
        // Validate the witnessed Canon snapshot
        observed.validate()?;

        // Perform Holylog seal on the active loglet
        let historical = resolve_read_seal(parts.components(k)).await?;
        if !historical.observe_durable().await?.sealed() {
            historical.seal().await?;
        }
        ...
        // Provision successor - without installing it in the resolver yet
        ...
        // Witnessed Canon CAS reconfiguration
        let outcome = self
            .virtual_log()
        ...
        match outcome {
            ReceiptReconfiguration::Applied { .. } => {
                // Install the successor writable state only after reconfiguration is applied
                self.resolver
            ...
            ReceiptReconfiguration::Conflict { receipt } => {
                // Do not install the successor, keep it non-serving and inspectable
                Ok(VerseControlOutcome::ConflictNeedsInspect {
                    candidate: AbandonedProvisionCandidate {
                        receipt,
                        writable,
...
pub enum SupervisorError {
    ...
    /// The observed durable tail was not zero for greenfield activation.
    #[error(
    ...
    /// Greenfield activation disposition is not RecoveryRequired::MustSealAndReplace.
    #[error(
...
mod tests {
    ...
    use super::{
        NodeIdentity, PartsFactory, SharedMemoryPartsFactory, SupervisorError, VerseControlOutcome,
        VerseNodeSupervisor,
    };
    ...
    #[tokio::test]
    async fn activate_empty_open_generation_success() {
        let register = Arc::new(InMemoryConditionalRegister::new());
        let parts = Arc::new(SharedMemoryPartsFactory::default());
        ...
        // Bootstrap canon and exit
        node.bootstrap_canon(first.clone(), 2)
            .await
            .expect("bootstrap");

        // Simulate new process start by dropping process local state
        node.drop_process_local_state().await.expect("drop");

        // Start configured - must return RecoveryRequired
        let outcome = node
            .start_configured(SystemClock::new(), scripture::SystemTimer::new(), 2)
            .await
            .expect("start");
        ...
        // Activate empty open generation - must succeed and serve
        let outcome = node
    ...
    #[tokio::test]
    async fn activate_empty_open_generation_rejects_non_empty_tail() {
        let register = Arc::new(InMemoryConditionalRegister::new());
        let parts = Arc::new(SharedMemoryPartsFactory::default());
        let claims = Arc::new(InMemoryExclusiveClaimStore::new());
        let resolver = Arc::new(ProcessLogletResolver::default());
        ...
        // Write a record to make the tail non-empty
        node.virtual_log()
            .append(bytes::Bytes::from_static(b"hello"))
            .await
            .expect("append");

        // Crash the active writer to simulate loss
        node.crash_active_writer().await.expect("crash");

        // Re-start configured - must return RecoveryRequired
        let outcome = node
            .start_configured(SystemClock::new(), scripture::SystemTimer::new(), 2)
            .await
            .expect("start");
        ...
        // Try activating - must fail closed with NonEmptyTail error
        let err = node
            .activate_empty_open_generation(
                second.clone(),
                SystemClock::new(),
        ...
        assert!(matches!(err, SupervisorError::NonEmptyTail { tail: 1 }));

        // Verify it was not installed and first remains the active loglet
        assert!(!resolver.contains(&second));
        assert_eq!(node.active_loglet().await.expect("active"), first);
    }

    #[tokio::test]
    async fn activate_empty_open_generation_rejects_foreign_owner_and_standby() {
        let register = Arc::new(InMemoryConditionalRegister::new());
        let parts = Arc::new(SharedMemoryPartsFactory::default());
        let claims = Arc::new(InMemoryExclusiveClaimStore::new());
        let resolver_a = Arc::new(ProcessLogletResolver::default());
        ...
        let resolver_b = Arc::new(ProcessLogletResolver::default());
        ...
        // Bootstrap on node_a (owner_a is the owner)
        node_a
            .bootstrap_canon(first.clone(), 2)
            .await
            .expect("bootstrap");

        // Start node_b - must return Standby because it is owned by owner_a
        let outcome = node_b
            .start_configured(SystemClock::new(), scripture::SystemTimer::new(), 2)
            .await
            .expect("start");
        ...
        // node_b tries to activate successor - must fail closed because it is not the owner
        let err = node_b
            .activate_empty_open_generation(
                second.clone(),
                SystemClock::new(),
        ...
        assert!(matches!(
            err,
            SupervisorError::CanonAuthority(scripture::CanonAuthorityError::NotOwner { .. })
        ));

        assert!(!resolver_b.contains(&second));
    }

    #[tokio::test]
    async fn activate_empty_open_generation_rejects_stale_or_competing() {
        let register = Arc::new(InMemoryConditionalRegister::new());
        let parts = Arc::new(SharedMemoryPartsFactory::default());
        let claims: Arc<dyn ExclusiveClaimStore> = Arc::new(InMemoryExclusiveClaimStore::new());

        let left = VerseNodeSupervisor::with_parts_factory_and_claims(
            identity(owner_a()),
            Arc::clone(&register) as Arc<dyn holylog::virtual_log::ConditionalRegister>,
            Arc::new(ProcessLogletResolver::default()),
            Arc::clone(&parts) as Arc<dyn super::PartsFactory>,
        ...
        let right = VerseNodeSupervisor::with_parts_factory_and_claims(
            identity(owner_a()),
            Arc::clone(&register) as Arc<dyn holylog::virtual_log::ConditionalRegister>,
            Arc::new(ProcessLogletResolver::default()),
            Arc::clone(&parts) as Arc<dyn super::PartsFactory>,
        ...
        left.bootstrap_canon(first.clone(), 2)
            .await
            .expect("bootstrap");

        left.drop_process_local_state().await.expect("drop left");
        right.drop_process_local_state().await.expect("drop right");
        ...
        let left_task = Arc::new(left);
        let right_task = Arc::new(right);

        let (left_out, right_out) = tokio::join!(
            left_task.activate_empty_open_generation(
                LogletId::new("succ-l").expect("id"),
                SystemClock::new(),
                scripture::SystemTimer::new(),
                2,
            ),
            right_task.activate_empty_open_generation(
                LogletId::new("succ-r").expect("id"),
                SystemClock::new(),
                scripture::SystemTimer::new(),
                2,
            ),
        ...
        for result in [&left_out, &right_out] {
            match result {
                Ok(VerseControlOutcome::Serving) => serving += 1,
                Ok(VerseControlOutcome::ConflictNeedsInspect { .. }) => expected_fail += 1,
                Err(SupervisorError::StaleActive { .. }) => expected_fail += 1,
                Err(SupervisorError::InvalidActivationDisposition {
                    disposition: Err(OpenReattachError::AlreadySealed { .. }),
                }) => expected_fail += 1,
                other => panic!("unexpected replace outcome: {other:?}"),
            }
        }
        assert_eq!(serving, 1);
        assert_eq!(expected_fail, 1);
    }

    #[tokio::test]
    async fn activate_empty_open_generation_rejects_already_sealed_or_malformed() {
        let register = Arc::new(InMemoryConditionalRegister::new());
        let parts = Arc::new(SharedMemoryPartsFactory::default());
        let claims = Arc::new(InMemoryExclusiveClaimStore::new());
        let resolver = Arc::new(ProcessLogletResolver::default());
        ...
        // Attempting to activate must fail closed because disposition is AlreadySealed, not MustSealAndReplace
        let err = node
            .activate_empty_open_generation(
                second.clone(),
                SystemClock::new(),
