...
//!
//! Unlike [`super::SpoolCell`] (a Scribe-local pre-commit WAL that poisons on
...
//!   synced** (`fsync` on [`super::FileSpoolStorage`]).
...
use crate::driver::{Receipt, Submission};
use crate::model::JournalId;

use super::file::FileSpoolStorage;
use super::frame::SpoolFrame;
use super::progress::ProgressIdentity;
use super::storage::{InMemorySpoolStorage, SpoolError, SpoolStorage, ValidFrame};
    ...
    for valid in frames {
        match &valid.frame {
            ...
            SpoolFrame::PreCommit { .. } => {
                // Pre-commit frames are Scribe-local WAL; ignore for producer continuity.
            }
        }
    }
...
mod tests {
    ...
    use crate::{ProducerId, Record, Submission};
    use bytes::Bytes;
    ...
    fn file_outbox_survives_reopen_with_pending_and_progress() {
        ...
        assert!(
            snap.committed
                .contains(&ContinuityId::from_submission(&submission(0)))
        );
